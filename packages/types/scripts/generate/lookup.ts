...
function generateTypeDocs(
	...
	return `${id ? `${registry.createLookupType(id)}${path.length ? ": " : ""}` : ""}${path
		.map((p) => p.toString())
		.join("::")}${
		params.length ? `<${params.map((p) => generateParamType(registry, p)).join(", ")}>` : ""
	}`;
}
	...
	writeFile(path.join(destDir, `${subPath || "definitions"}.ts`), (): string => {
		...
		return (subPath ? generateLookupDefsNamedTmpl : generateLookupDefsTmpl)({
			defs: all.map(({ docs, type }, i) => {
				...
				return {
					defs: [[typeName || typeLookup, `${def}${i !== max ? "," : ""}`]].map(
						([n, t]) => `${n}: ${t}`
					),
					docs,
				};
...
function generateLookup(destDir: string, entries: [string, HexString][]): void {
	entries.reduce<string[]>((exclude, [subPath, staticMeta]): string[] => {
		...
		return exclude.concat(
			...filtered.map(([, typeDef]) => typeDef.lookupName).filter((n): n is string => !!n)
		);
	}, []);
}
...
export function generateDefaultLookup(
	destDir = "packages/types-augment/src/lookup",
