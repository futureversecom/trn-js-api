...
export function main() {
	...
	const filteredBase = Object.entries(
		substrateDefinitions as Record<string, unknown>,
	)
		.filter(([key]) => {
			if (userKeys.includes(key)) {
				console.warn(
					`Override found for ${key} in user types, ignoring in @polkadot/types`,
				);
	...
	generateDefaultErrors(
		path.join(augmentPath, "api-errors.ts"),
		metadata,
		allDefs,
		false,
	);
