...
const generateLookupDefsNamedTmpl = Handlebars.compile(
	readTemplate("lookup/defs-named"),
);
const generateLookupIndexTmpl = Handlebars.compile(
	readTemplate("lookup/index"),
);
const generateLookupTypesTmpl = Handlebars.compile(
	readTemplate("lookup/types"),
);
const generateRegistryTmpl = Handlebars.compile(
	readTemplate("interfaceRegistry"),
);

function generateParamType(
	registry: Registry,
	{ name, type }: SiTypeParameter,
): string {
	if (type.isSome) {
...
function generateTypeDocs(
	...
	return `${id ? `${registry.createLookupType(id)}${path.length ? ": " : ""}` : ""}${
		path.map((p) => p.toString()).join("::")
	}${
		params.length
			? `<${params.map((p) => generateParamType(registry, p)).join(", ")}>`
			: ""
	}`;
}
...
function expandObject(
	parsed: Record<string, string | Record<string, string>>,
): string[] {
	if (parsed._set) {
...
function expandType(encoded: string): string[] {
	...
	return expandObject(
		JSON.parse(encoded) as Record<string, string | Record<string, string>>,
	);
}

function expandDefToString(
	{ lookupNameRoot, type }: TypeDef,
	indent: number,
): string {
	if (lookupNameRoot) {
...
function getFilteredTypes(
	...
	exclude: string[] = [],
): [PortableType, TypeDef][] {
	const named = lookup.types.filter(
		({ id }) => !!lookup.getTypeDef(id).lookupName,
	);
	const names = named.map(({ id }) => lookup.getName(id));

	return named
		.filter(
			(_, index) => !names.some((n, iindex) => index > iindex && n === names[index]),
		)
		.map((p): [PortableType, TypeDef] => [p, lookup.getTypeDef(p.id)])
		.filter(
			([, typeDef]) => !exclude.includes(typeDef.lookupName || "<invalid>"),
		);
}
...
function generateLookupDefs(
	...
	writeFile(
		path.join(destDir, `${subPath || "definitions"}.ts`),
		(): string => {
...
function generateLookupTypes(
		...
		.map(([, typeDef]) => {
			...
			return typeDef.lookupNameRoot && typeDef.lookupName
				? exportInterface(
					typeDef.lookupIndex,
					typeDef.lookupName,
					typeDef.lookupNameRoot,
				)
				: typeEncoders[typeDef.info](
					registry,
					imports.definitions,
					typeDef,
					imports,
	...
	writeFile(
		...
		() => generateLookupIndexTmpl({ headerType: "defs" }),
		true,
	);
}
	...
	writeFile(
		...
		(): string => {
			const items = filtered
				...
				.sort()
				.reduce(
					(all: string[], n) => (all.includes(n) ? all : all.concat(n)),
					[],
				);
...
function generateLookup(destDir: string, entries: [string, HexString][]): void {
	entries.reduce<string[]>((exclude, [subPath, staticMeta]): string[] => {
		...
		generateRegistry(
			...
			filtered,
			destDir,
			subPath === "lookup" ? "registry" : `../registry/${subPath}`,
		);
...
export function generateDefaultLookup(
	destDir: string,
	staticData: HexString,
): void {
	generateLookup(destDir, [["rootnet", staticData]]);
}
